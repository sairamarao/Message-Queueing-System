...
public class ReceiverPopHandler {
	...
	public void receiveMessage() {
		String message = "receiveMessage "+ReceiverId+" 1"+(char)13;
		try {
			long w0 = System.currentTimeMillis();
			osw.write(message);
			osw.flush();
	...
	public void receiver(int id,String MWhost){
		try {
			...
			Thread throughput = new Thread(new Runnable() {
				public void run() {
						while(ReceiverPopHandler.this.throughputThread == 1){
							try{
								Thread.sleep(1000);
								System.out.println("Debugging.."+ReceiverPopHandler.this.ClientId);
							}catch(InterruptedException ie){}
							ReceiverPopHandler.this.twriter.println(ReceiverPopHandler.this.reqSent+" "+ReceiverPopHandler.this.resRec);
						}
				}
			});
			throughput.start();
